...
amsApp.controller('welcomeController', function($rootScope,$scope,$state, $stateParams,  $sce, $http, $location, accountServices) {
...
amsApp.factory('accountServices',['$http','$rootScope','$window',function($http, $rootScope,$window){
		var fac = {};
		
		fac.logoffUser = function(){
			$rootScope.acc_token = '';
			var configs = {};
			fac.httpCall("config/configParams.json").then(function successCallback(response) {
		...
		fac.getConfigParams = function(){
			//var configs={};
			return fac.httpCall("config/configParams.json");
